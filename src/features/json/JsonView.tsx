...
import { jsonList, jsonGet, jsonSet, jsonDelete, jsonHistory } from "../../lib/json";
import type { VersionedValue } from "../../lib/kv";
import { toPlain, fromPlain } from "../../lib/value";
...
export function JsonView() {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      setKeys(await jsonList(handle, branch, space));
    } catch (e) {
      setError(String(e));
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    jsonGet(handle, selected, branch, space)
  ...
  async function save() {
    ...
    try {
      await jsonSet(handle, key, value, branch, space);
      await refresh();
      setSelected(key);
      setMode("view");
    } catch (e) {
  ...
  async function del() {
    ...
    try {
      await jsonDelete(handle, selected, branch, space);
      setSelected(null);
      setDeleting(false);
      await refresh();
    } catch (e) {
  ...
  async function openHistory() {
    if (handle === null || selected === null) return;
    try {
      const h = await jsonHistory(handle, selected, branch, space);
      setHistory(h);
  ...
  async function restore(v: VersionedValue) {
    if (handle === null || selected === null) return;
    try {
      await jsonSet(handle, selected, v.value, branch, space);
      await refresh();
      const h = await jsonHistory(handle, selected, branch, space);
      setHistory(h);
      setHistSel(h[0]?.version ?? null);
      setDetail(await jsonGet(handle, selected, branch, space));
    } catch (e) {
      setError(String(e));
    }
  }
    ...
    <div className="feature">
      <header className="feature-head">
        ...
        <span className="muted">
          {shown.length} / {keys.length} docs · <code>{branch}</code> / <code>{space}</code>
        </span>
        <span className="grow" />
      ...
      <div className="split">
        <div className="list-pane">
          ...
          {!loading && !error && shown.length === 0 && <div className="muted pad">No documents</div>}
        ...
        <div className="detail-pane">
          {mode === "add" || mode === "edit" ? (
            <div className="editor">
              ...
              <span className="dk">document (JSON)</span>
              <textarea
            ...
            <div className="muted pad">Select a document to inspect</div>
          ) : detail === null ? (
            <div className="muted pad">Document not found in this branch / space</div>
          ) : (
            <>
              <div className="detail-row">
                <span className="dk">key</span>
